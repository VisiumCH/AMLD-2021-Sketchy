...
def get_classes(tsv_path, sketch_emb):
    """
...
def get_tiles(im_path, embedding_number):
...
def project_embeddings(tensors, nb_dimensions, reduction_algo, sketch_emb):
    ...
    if reduction_algo == "PCA":
        ...
        pca.fit(tensors)
        tensors = pca.transform(tensors)
    elif reduction_algo == "TSNE":
        tensors = TSNE(n_components=nb_dimensions).fit_transform(tensors)
    else:
        raise Exception(f"Reduction algorithm {reduction_algo} not implemented.")
    return tensors


def process_graph(embeddings_path, nb_dimensions, reduction_algo, sketch_emb=False):
    """Retrieve all the data from the files generated by tensorboard"""
    # File names
    tensors_path = embeddings_path + TENSORBOARD_EMBEDDINGS
    tsv_path = embeddings_path + TENSORBOARD_CLASSES

    tensors = read_tensor_tsv_file(tensors_path)
    X = project_embeddings(tensors, nb_dimensions, reduction_algo, sketch_emb)
    classes = get_classes(tsv_path, sketch_emb)
